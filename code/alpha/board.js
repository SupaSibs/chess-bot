...
var Board = /** @class */ (function () {
    ...
    Board.prototype.checkCollisions = function (direction, oldX, oldY, newX, newY, colour) {
        var directionFunc = function () { };
        switch (direction) {
            case 1: //top right
                directionFunc = function (x, y, i) { return { x: x + i, y: y + i }; };
                break;
            case 2: //top left
                directionFunc = function (x, y, i) { return { x: x - i, y: y + i }; };
                break;
            ...
            case 4: //bottom left
                ...
                break;
            case 5: //right
                directionFunc = function (x, y, i) { return { x: x + i, y: y }; };
                break;
            case 6: //left
                directionFunc = function (x, y, i) { return { x: x - i, y: y }; };
                break;
            case 7: //up
                directionFunc = function (x, y, i) { return { x: x, y: y + i }; };
                break;
            case 8: //down
                directionFunc = function (x, y, i) { return { x: x, y: y - i }; };
                break;
        ...
        for (var i = 0; i <= 8; i++) {
            ...
            if (this.position[xy.x][xy.y].constructor.name != "Empty" && this.position[xy.x][xy.y].colour == colour) {
                pieceDetected = true;
            }
            if (pieceDetected) {
    ...
    Board.prototype.validateMove = function (square, move, colour) {
        ...
        if (this.position[oldX][oldY].validateMove(move) == false || collisionTest.detection == true && collisionTest.squareDetected == alphabet[newX] + newY) {
            validation = true;
        }
        for (var i = 0; i <= 8; )
            : i++;
